...
/// <remarks>
/// Implements Campanus house circles through the north and south points of the horizon.
/// Written from this geometric definition with a numeric oracle as the only reference.
/// </remarks>
internal static class CampanusHouses
{
        ...
        Func<double, double, List<HouseCusp>> porphyryFallback)
    {
        ...
        cusps[1] = CuspLongitude(north, latitude, 120.0, ramc);
        cusps[2] = CuspLongitude(north, latitude, 150.0, ramc);
        cusps[3] = AstroMath.NormalizeDegrees(mcLongitude + 180.0);
        cusps[6] = AstroMath.NormalizeDegrees(ascLongitude + 180.0);
        cusps[9] = mcLongitude;
        cusps[10] = CuspLongitude(north, latitude, 30.0, ramc);
        cusps[11] = CuspLongitude(north, latitude, 60.0, ramc);
        return QuadrantCusps.CompleteOpposites(cusps);
    }

    private static double CuspLongitude(
        HouseCircleGeometry.Vector north,
        double latitude,
        double distanceFromZenith,
        double ramc)
        => HouseCircleGeometry.IntersectionLongitudeOnMcArc(
            north,
            HouseCircleGeometry.PrimeVerticalPoint(latitude, distanceFromZenith),
            ramc,
            Obliquity);
}
