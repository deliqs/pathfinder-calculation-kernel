...
public class RegiomontanusHousesInvariantTests
{
    private readonly HouseCalculator _calculator = new();

    [Theory]
    [MemberData(nameof(OracleRows))]
    public void CalculateHousesFromMc_PreservesRegiomontanusInvariants(
        QuadrantHouseOracleTests.OracleRow row)
    {
        var houses = _calculator.CalculateHousesFromMc(row.Mc, row.Latitude, HouseSystem.Regiomontanus);
    ...
    public void HorizonCircle_IntersectsAtTheAscendant()
    {
        ...
        var longitude = HouseCircleGeometry.IntersectionLongitudeOnMcArc(
            ...
            HouseCircleGeometry.EquatorPoint(90.0),
            ramc,
            23.4393);
    ...
    [InlineData(66.0)]
    [InlineData(-66.0)]
    public void RegiomontanusAtThreshold_IsComputedRatherThanPorphyry(double latitude)
        => AssertComputedAtThreshold(latitude);

    [Theory]
    [InlineData(66.5)]
    [InlineData(-66.5)]
    public void RegiomontanusBeyondThreshold_FallsBackToPorphyry(double latitude)
        => AssertFallback(latitude);

    public static IEnumerable<object[]> OracleRows() => QuadrantHouseOracleTests.McOracleRows()
        .Where(row => ((QuadrantHouseOracleTests.OracleRow)row[0]).System == HouseSystem.Regiomontanus);

    private void AssertComputedAtThreshold(double latitude)
    {
        var computed = _calculator.CalculateHousesFromMc(99.0, latitude, HouseSystem.Regiomontanus);
    ...
    {
        var actual = _calculator.CalculateHousesFromMc(99.0, latitude, HouseSystem.Regiomontanus);
