...
The four archived Swiss stdout files contain generated numeric table output, and one minimal derived version-identification line records the program identity; the first-party manifest records the corresponding invocations. They contain no Swiss source code, executable, or ephemeris data files. They remain attributed to Swiss Ephemeris and are not relicensed under MIT.
